...
public struct MUFlowLayout: Layout {
...
#if DEBUG
    #Preview {
        @Previewable @State var alignment: Alignment = .leading
        VStack(spacing: 16) {
            ScrollView {
                MUFlowLayout(alignment: alignment) {
                    ForEach(0 ..< 100, id: \.self) { _ in
                        Rectangle()
                            .fill(Color.teal)
                            .frame(width: CGFloat.random(in: 50 ... 200), height: 50)
                    }
                }
            }
